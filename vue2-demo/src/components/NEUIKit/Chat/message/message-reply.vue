<template>
  <div v-if="visible" class="reply-msg-wrapper">
    ...
    <div v-if="replyMsg && replyMsg.messageClientId == 'noFind'">
    ...
    <Popover
      ...
      @update:modelValue="(val) => (popoverVisible = val)"
    >
      ...
      <div class="reply-msg">
        <div class="reply-msg-name-wrapper">
          <div class="reply-msg-name-content">
            <Appellation
              ...
              :teamId="replyMsg && replyMsg.receiverId"
              :font-size="13"
              color="#666666"
            />
            :
        ...
        <div class="reply-msg-type-tip" v-else>
          {{
            replyMsg && replyMsg.messageType
              ? "[" +
                (replayMsgTypeMap[replyMsg.messageType] ||
                  t("unknownMsgText")) +
                "]"
              : "[Unknown]"
          }}
      ...
      <template #content>
        <div @click.stop class="popover-message-content">
...
//将 MessageItemContent 在 message-reply.vue 中改为异步组件注册，打破循环依赖。
const MessageItemContent = () => import("./message-item-content.vue");
...
export default {
  ...
  computed: {
    ...
    replyMsgType() {
      return this.replyMsg && this.replyMsg.messageType;
    },
  },
  data() {
    return {
