...
export const getAvatarBackgroundColor = (account) => {
  const colorMap = {
    0: "#60CFA7",
    1: "#53C3F3",
    2: "#537FF4",
...
export function detectFramework() {
  try {
    ...
    if (vue && typeof vue.version === "string") {
      if (vue.version.startsWith("2.")) return "Vue2";
      if (vue.version.startsWith("3.")) return "Vue3";
    }

    const react = g && g.React;
    if (react && typeof react.version === "string") {
      return "React";
    }
    ...
    const isVue3El = (el) => !!(el && (el.__vue_app__ || el.__vueParentComponent));
    const isVue2El = (el) => !!(el && el.__vue__);
    const isReactEl = (el) => {
      if (!el) return false;
    ...
    roots.forEach((r) => {
      candidates.push(r);
      if (r && r.firstElementChild) candidates.push(r.firstElementChild);
      if (r && r.firstElementChild && r.firstElementChild.firstElementChild) {
        candidates.push(r.firstElementChild.firstElementChild);
      }
    });

    if (candidates.some(isVue3El)) return "Vue3";
    if (candidates.some(isVue2El)) return "Vue2";
    if (candidates.some(isReactEl)) return "React";

    const reactRoot =
      doc.querySelector && doc.querySelector("[data-reactroot],[data-reactid]");
    if (reactRoot) return "React";
